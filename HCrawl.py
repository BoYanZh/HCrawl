...
STOP_FLAG = False
DOMAIN = "https://tuav42.com/"
SITE_NAME = DOMAIN.split(".")[-2].split("/")[-1]
HOME_PAGE = "https://tuav42.com/"
PAGE_KEY_WORD = "欧美色图"
MAX_THREAD_COUNT = 16
...
def getIndexPageDict(html):
    ...
    for link in soup.find_all('a'):
        if (link.string is not None and PAGE_KEY_WORD in link.string):
            tmpDict = {
                "keyWord": link.string,
...
def getContentPageDictList(indexPageDict):
    ...
    for link in soup.find_all('a'):
        ...
        if (tmpDict["name"] is not None and len(tmpDict["name"]) > 6
...
def getImgUrlDictList(contentPageDict):
    ...
    filePath = os.path.join(
        SITE_NAME, contentPageDict["keyWord"].strip(),
        contentPageDict["name"].strip())
    ...
    for link in soup.find_all('img'):
        tmpDict = {
            "keyWord": contentPageDict["keyWord"],
...
def downloadImg(imgUrlDict):
    global DOMAIN, SITE_NAME
    if ("http" not in imgUrlDict["url"]):
        imgUrlDict["url"] = DOMAIN + imgUrlDict["url"]
    filePath = os.path.join(
        SITE_NAME, imgUrlDict["keyWord"].strip(),
        imgUrlDict["name"].strip())
...
def getNextIndexPageDict(indexPageDict):
    ...
    soup = BeautifulSoup(html, "lxml")
    for link in soup.find_all('a'):
        if (link.string is not None and "下一页" in link.string):
            tmpDict = {
                "keyWord": indexPageDict["keyWord"],
                "url": link.get('href').replace(DOMAIN, "")
            }
